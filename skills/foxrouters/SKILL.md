...
| **Version** | v1.6.1 (local; check live: `curl -s http://127.0.0.1:20130/health`) |
...
- **kimi-k3 now on CodeBuddy upstream** (2026-07-20) — `kimi-k3` previously returned `{"code":11102,"msg":"model [kimi-k3] service info not found"}` from www.codebuddy.ai/v2. As of 2026-07-20, it returns 200 OK and is now hardcoded into the default model list (`internal/proxy/proxy.go` alongside `kimi-k2.5`). No need to register via custom-models API anymore. If upstream disables it again, requests will surface the 11102 error to the client.
